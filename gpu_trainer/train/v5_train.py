...
def run_v5_forward_test(
    ...
    _er_skip_counts: dict = {}   # symbol -> signals skipped since blocking
        ...
        if config.rolling_er_gate and test_sym_ids is not None:
            ...
            if _er_sym and _er_sym in _er_blocked:
                ...
                if _er_skip_counts[_er_sym] < config.rolling_er_window:
                    # Still within cooldown — skip this signal.
                    ...
                    continue
                # Cooldown window elapsed — grant a probe trade opportunity.
                # The probe trade's realized R will update the deque and
                # determine whether to re-block (still bad E[R]) or stay unblocked.
                _er_blocked.discard(_er_sym)
                _er_skip_counts[_er_sym] = 0
                log.info(
                    "[V5_FWD][ER_GATE_PROBE] %s: cooldown elapsed (%d signals skipped) "
                    "— allowing probe trade; E[R] will be re-evaluated on close",
                    _er_sym, config.rolling_er_window,
                )
                # Fall through — let the signal be processed normally.

...
Exit codes:
  0 — at least 1 fold with ACTIVE trades (E[R]>0) AND score_monotonic=True
  1 — all symbols HIGH_BAR / no trades / no monotone fold
...
Usage (run from gpu_trainer directory):
    ...
    # With a model-path hint (informational — walk-forward always re-trains):
    python scripts/validate_signals.py --model-path checkpoints/v5_run01 \\
...
def _run_validation(args):
    ...
    if args.model_path:
        log.info(
            "Model path : %s  (informational — walk-forward re-trains fresh weights)",
            args.model_path,
        )
    log.info("=" * 80)
    ...
    for i, f in enumerate(folds):
        ...
        if sq:
            long_pct = sq.get("long_avg_p_side", 0)
            short_pct = sq.get("short_avg_p_side", 0)
            log.info(
        ...
        if n_tr > 0 and er > 0 and mono is True:
    ...
    if per_sym:
        ...
        for sym, info in per_sym.items():
            ...
            log.info(
                ...
                sym, n, er, wr * 100, folds_seen, total_r_sym, status_tag,
            )
        log.info("")
        log.info(
            "  Active   : %d — %s",
    ...
    if total_trades == 0:
        ...
        log.warning(
            "WARN: Trades generated (%d) but no fold has E[R]>0 AND score_monotonic=True.",
            total_trades,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Optional: path to a prior checkpoint directory for reference/logging. "
             "Walk-forward always re-trains from scratch; this is informational only.",
    )

...
export default function LiveTrading() {
  ...
  useEffect(() => {
    const unsub = subscribe("EXECUTION_STATE", () => {
      queryClient.invalidateQueries({ queryKey: ["/api/bybit/positions"] });
      queryClient.invalidateQueries({ queryKey: ["/api/bybit/balance"] });
    });
    return unsub;
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              {bybitStatus?.executionService?.connected && (
                <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[10px]" data-testid="badge-exec-service">
                  Execution Service
                </Badge>
              )}
            </div>

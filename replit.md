...
Training enhancements to improve binary enter quality prediction. Focal BCE loss (gamma=1.5, alpha=0.60) replaces standard BCE, focusing gradients on hard-to-classify samples. Online Hard Example Mining (OHEM) keeps all positives and top 35% hardest negatives per batch. An auxiliary edge regression head (edge = mfe_r - mae_r, weight=0.3) provides trade-quality gradient signal. Soft quality labels (optional, --use-soft-labels) use sigmoid(edge/temp) instead of hard 0/1. ECE (Expected Calibration Error) computed with 15 bins before/after temperature scaling and saved in temp_scale_v5.0.json. Promotion gate min PR-AUC raised to 0.42 (from 0.35). CLI flags: --use-focal-loss/--no-focal-loss, --focal-gamma, --focal-alpha, --use-ohem/--no-ohem, --ohem-neg-pct, --use-edge-head/--no-edge-head, --edge-loss-weight, --use-soft-labels, --soft-label-temp, --promote-min-pr-auc, --verify-pr-auc-upgrade. Training config saved in checkpoints for reproducibility.

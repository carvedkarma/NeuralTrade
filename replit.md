...
- `/paper` — Paper Trading: Enable/disable toggles, portfolio metrics, equity curve, signal-strength leverage tiers display (6 tiers: 1x/2x/3x/5x/10x/30x), open/closed positions (with close/partial-close/edit SL-TP actions, live 1s price updates), **position health gauges** (0-100 score with color-coded risk levels), **neural status badges** (shows latest Neural PM action), **MFE tracker** (peak profit vs giveback), **breakeven indicator**, **adaptive trail visualization**, configuration
...
- **API Routes** (`server/routes.ts`):
  ...
  - `POST /api/live/cycle-log` — Receive cycle log from GPU trainer, broadcasts via WebSocket (includes v5Score, v5Threshold, v5Side). **Auto-trade**: when decision=ENTER and paper trading is enabled, automatically opens a paper position via `manualOpenPosition` with source="v5_signal", SL/TP from GPU trainer's ATR-based calculation (sl_price/tp_price fields in cycle log payload), with server-side validation (finite, positive, correct side orientation). Falls back to 1.5% stop distance with 2:1 R:R if GPU SL/TP not provided or invalid. Safety checks: max 1 position per symbol, max 6 total open positions. **Signal-strength leverage**: v5Score is passed to `computeSignalLeverage()` which maps score to leverage tiers (≥0.02→1x, ≥0.05→2x, ≥0.10→3x, ≥0.15→5x, ≥0.40→10x, ≥0.50→30x). Position qty is multiplied by leverage; risk per trade stays constant.

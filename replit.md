...
Fixes reversed logit polarity (mean_logit_neg > mean_logit_pos) that caused PR-AUC collapse. Changes: (A) Separation margin increased from 0.20 to 0.30 with stronger weight (0.03, was 0.02); (B) Flip penalty added: 0.05 * relu(mean_neg - mean_pos) applies extra gradient when logits are reversed; (C) Focal alpha reduced from 0.40 to 0.35 to further reduce positive class overweighting; (D) Per-epoch [SEP_CHECK] log line shows mean_pos, mean_neg, sep for directional monitoring; (E) Version bumped to v4.5.2_directional_sep_fix. No changes to labels, horizon, data pipeline, model structure, OHEM, or edge/value heads.

### v4.5.1 PR-AUC Stable Patch
Stabilization patch for the PR-AUC Upgrade Pack, fixing Pred% collapse to 0%/100% and PR-AUC stalling at ~0.09-0.26. Changes: (A) Soft labels ON by default (temp=1.5 down from 2.0) with --no-soft-labels toggle; (B) OHEM disabled by default (was True) — focal + OHEM + high pos_weight caused all-positive collapse; re-enable with --use-ohem if needed; (C) Horizon standardized to 16 bars across triple_barrier, model, and regression targets with [HORIZON_CHECK] verification log; (D) Label purity tightened: r_min_expiry raised from 0.5 to 1.0 to remove weak expiry wins; (E) OI auto-disable with coverage threshold: _oi_sanity_check returns oi_enabled bool, checks 30-day window coverage >= 80% — if below threshold, OI features zeroed out for that symbol with one-time [OI_CHECK] DISABLED log, funding remains enabled (100% coverage); (F) --verify-pr-auc-upgrade extended with 15 checks including soft_labels, ohem status, r_min_expiry, horizon alignment, OI auto-disable threshold; (G) Version bumped to v4.5.1_pr_auc_stable; (H) Focal alpha reduced from 0.60 to 0.40 and pos_weight capped at 2.5 (was 10.0) to reduce positive pressure; (I) Logit separation regularizer: 0.02 * relu(0.2 - sep) enforces minimum 0.2 logit gap between positives and negatives to prevent all-positive collapse; (J) Per-epoch [SAFETY] log line shows Pos%, Pred%, mean_logit_pos/neg, sep, PR-AUC for real-time monitoring.

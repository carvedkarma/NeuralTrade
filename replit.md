...
A three-stage loss schedule prevents ENTER classifier collapse. Stage 1: WARMUP (default 10 epochs) uses plain BCEWithLogitsLoss with capped pos_weight (default 2.0), no focal loss, no OHEM. Stage 2: TRANSITION (default 15 epochs) gradually ramps pos_weight from warmup value to final value and focal gamma from 0.0 to focal_gamma_final (default 0.5), with OHEM disabled by default. LR drops by 35% at WARMUP->TRANSITION boundary. Stage 3: FULL enables focal loss at final gamma, OHEM (if configured), and full pos_weight. A collapse guard monitors Pred1 and sep: if Pred1>98% for 2 consecutive epochs OR (sep<0.02 AND Pred1>95%), it automatically disables OHEM and clamps pos_weight<=2.5 for 5 epochs. CLI flags: --loss-warmup-epochs, --warmup-pos-weight, --transition-epochs, --focal-gamma-final, --lr-drop-on-transition, --collapse-guard, --collapse-guard-pred1, --collapse-guard-sep, --collapse-guard-freeze-epochs. Each epoch logs [LOSS_STAGE] with stage=WARMUP/TRANSITION/FULL, pos_weight, focal_gamma, ohem status, and LR. All transition/collapse settings saved to checkpoint config.
...
The v4.7 "Label Geometry Fix" addresses overly permissive labeling (~90% positive rate) by implementing strict ENTER=1 criteria. v4.7.1 "TP Quality Score Balancing" replaces the degenerate r_min_enter threshold (best_R capped at tp_mult/sl_mult=1.3333 for all TP hits) with a continuous TP quality score. For TP-first bars: q = 0.50*(1 - t_hit/horizon) + 0.25*clamp(mfe/tp_dist, 0, 1) + 0.25*(1 - clamp(mae/sl_dist, 0, 1)). ENTER=1 requires: (A) TP-first AND q >= q_min_tp, OR (B) expiry with R >= r_min_expiry_strict (default 1.0). Auto-balance searches q_min_tp in [0.0, 0.9] to hit target_enter_rate (~18%). triple_barrier_outcome_detailed() returns t_hit, MFE, MAE diagnostics per side. CLI: --q-min-tp (default 0.3). Logs: [LABEL_V471], [TP_QUAL], [BEST_R_HIST]. All v4.7.1 config saved to checkpoints. Training-only — no live trading or dashboard modifications.

The v4.9.0 "Distributional Trade Forecaster" replaces binary ENTER classification with distributional outputs. The model produces: E[R] (expected R-multiple via value_head, Huber loss), quantile forecasts q10/q50/q90 (via dist_quantile_head, pinball loss), p(R>0) probability (via win_head, BCE loss), and optional regime classification chop/trend/highvol (via regime_head, CE loss). Trade selection uses score-based ranking: score = sigmoid(p_win) * E_R - lambda * max(0, -q10), where lambda (default 0.5) penalizes downside risk. Validation uses percentile-based sweep (top 5/10/15/20/25/30%) instead of threshold sweep, selecting the percentile with best expectancy within target trades/day (default 6.5 +/- 1.5). Composite loss = w_mse*Huber(E_R) + w_quantile*Pinball(q10/q50/q90) + w_bce*BCE(p_win) + w_dir*BCE(direction). No focal loss, OHEM, or collapse guard — the distributional approach eliminates binary classification entirely. Checkpoints save model_type='distributional_trade_forecaster' with all v4.9 config. Implemented as separate train_distributional_model() function preserving full backward compatibility with binary ENTER pipeline. CLI: --train-distributional, --dist-w-mse (1.0), --dist-w-quantile (0.5), --dist-w-bce (0.5), --dist-w-regime (0.0), --dist-w-dir (0.3), --score-lambda (0.5), --use-regime-head, --dist-target-tpd (6.5), --dist-target-tpd-tol (1.5). Logs: [DIST_CONFIG], [DIST_DATA], [DIST_EPOCH], [QUANTILE_COV], [SCORE].

The v4.9.1 "Enhanced Distributional Trade Forecaster" extends v4.9.0 with four opt-in subsystems, all backward-compatible via feature flags: (1) Candidate Engine (`--use-candidates`): filters bars by ATR/vol threshold (min 0.15%), chop rejection (ADX<18 + ATR_rank<0.20), breakout trigger (n-bar high/low), mean-reversion (zscore>=2.0), and fee gate (expected_move > round-trip cost). Auto-relaxes if candidate rate falls below 10%. Implemented in `gpu_trainer/data/candidate_generator.py`. (2) Multi-Preset Barriers (`--multi-preset`): evaluates 4-6 TP/SL presets (tight 1.5x/1.0x, standard 2.0x/1.5x, wide 3.0x/1.5x, asymmetric 3.5x/1.25x, swing 4.0x/2.0x, scalp 1.2x/0.8x) per bar, selects best R per bar. Uses `generate_multi_preset_targets()` in regression_targets.py. CLI: --barrier-presets (default: tight,standard,wide,asymmetric). (3) Money-Score formula (`--money-score`): replaces base score with efficiency-weighted formula: base = p_win * q50 - lambda * max(0, -q10), efficiency = base / (horizon_bars + 1), with optional regime weighting. (4) Risk Controls: daily loss limit (`--daily-loss-limit`, default -3.0R), max concurrent trades (`--max-concurrent`, default 6), per-symbol exposure cap (`--max-symbol-exposure`, default 3). Kelly-like position sizing (`--kelly-sizing`): kelly_f = p_win * avg_win/|avg_loss| - (1-p_win), size = clip(0.25 * kelly_f, 0.0005, 0.003). Multi-Horizon support (`--multi-horizon`): trains separate models per horizon (default 8,16,32 bars). Checkpoints save version='v4.9.1' with full v491_config. All features disabled by default; v4.9.0 behavior preserved when no flags set.

...
- **v5.0.9+ Threshold Ceiling Cap**: Added `--v5-max-threshold` to cap the calibrated threshold from above. Prevents sweep-calibrated thresholds from being set too high, which causes 0-trade forward test folds. When sweep calibrates e.g. 0.16 but model scores in forward test are lower, the ceiling cap (e.g. 0.10) ensures trades can still pass. Applied after floor logic but before percentile floor. Effective threshold = min(max(calibrated, floor), ceiling_cap).
